...
import { GoToRecipesClient } from "@/components/recipes/GoToRecipesClient";
...
export default async function GoToRecipesPage() {
    ...
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center gap-6 bg-[var(--bg)] p-6">
        <p className="max-w-md text-center font-sans text-sm text-muted-foreground">
          Sign in to manage your go-to recipes.
        </p>
  ...
  const userGoal = goalRow?.value?.trim() ?? "";
